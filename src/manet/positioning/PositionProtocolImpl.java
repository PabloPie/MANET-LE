...
public class PositionProtocolImpl implements PositionProtocol, EDProtocol {
	...
	private void move(Node host) {
		...
		double distance = this.getCurrentPosition().distance(this.current_destination);// en metres 10m
		double distance_to_next = (double) current_speed / 1000.0;// on le traduit en metre par milisecondes 8m/ms

		if (distance_to_next - distance < 0.0) { // Si on est pas encore arrivé après 1ms
		...
		if (current_position.equals(current_destination)) { // Si on est arrivé on se met en pause et on recalcule une nouvelle pos après la pause
			moving = false;

			EDSimulator.add(pause, loop_event, host, my_pid);
		} else { // Sinon on refait dans 1ms
